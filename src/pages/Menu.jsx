...
const Menu = ({ isOpen, onClose }) => {
  ...
  return (
    <div
      className={`fixed z-50 top-0 left-0 ${isOpen ? "w-full" : "w-0"
        } h-screen bg-[#02111B]  overflow-hidden`}
    >
      <button
        className="absolute top-4 right-4 sm:top-7 sm:right-8 p-2 text-white"
        onClick={onClose}
      >
        <CgClose size={24} />
      </button>
        ...
        <nav className="p-4 sm:p-6 md:p-8 overflow-y-auto h-full">
          <div className="flex flex-col lg:flex-row">
            <div className="text-left text-[#20D091] w-full lg:w-1/3 mb-8 lg:mb-0 lg:pr-8">
              ...
              <ul className="relative flex justify-start flex-col gap-6 sm:gap-10 md:gap-16 lg:gap-20">
                {navigationLinks.map((link, index) => (
                  <li
                    key={index}
                    className="text-2xl sm:text-3xl md:text-4xl lg:text-5xl font-bold transition-all delay-75 group flex items-center gap-3"
                  >
                    <div className="size-2 sm:size-3 bg-[#20D091] rounded-full scale-0 invisible group-hover:visible group-hover:scale-105 transition-all duration-300 ease-out"></div>
                    <Link
                      className="absolute group-hover:translate-x-4 sm:group-hover:translate-x-5 transition-all duration-300 ease-out"
                      to={link.path}
                      onClick={onClose}
                    >
                      {link.name}
